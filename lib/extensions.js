	...
	if(defaultconfigdir && installextconfigdir){
		...
		fs.mkdirs(installextconfigdir, function (err) {
			if (err) {
				...
				async.waterfall([
					function(cb){
						cb(null,{ ext_default_config_file_path : defaultconfigdir,
	...
	this.getExtensions({},function(err,currentExtensionsConf){
		if(err){
			...
			if(indexofext < indexofmovebefore){
				callback(null,{message:'MOVE BEFORE WARN: Extension ('+currentExtensionsConf[indexofext].name +') is already before ('+currentExtensionsConf[indexofmovebefore].name +')'});
			}
			else if(indexofmovebefore===false || indexofext === false){
				callback(null,{message:'MOVE BEFORE ERROR: trying to move extensions that are not in configuration'});
			}
			else{
				...
				if(extobj.name === extname){
					...
					callback(null,{message:'MOVE BEFORE ERROR: INVALID EXT CONFIG, MAY HAVE DUPLICATE EXTENSIONS'});	
				}
			}
		}
	}.bind(this));
