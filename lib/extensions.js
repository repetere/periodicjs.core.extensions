...
var Extensions = function (resources) {
			...
			extensionsConfig.extensions.forEach(function(ext,i){
				if(Object.keys(ext).length<6){
					...
					ext.periodicConfig.periodicDependencies.forEach(function(extDep){
						...
						else if(depPositionIndex<i && semver.ltr(extensionsConfig.extensions[depPositionIndex].version,extDep.version) && (typeof extDep.optional ==='undefined' || (typeof extDep ==='boolean' && extDep.optional ===true))){
	...
	this.init = function (appsettings) {
		...
		if (typeof appsettings !== 'undefined' && typeof appsettings.dirname !== 'undefined') {
			...
			if(extensionDirName.match('periodicjs/node_modules/@')){
				console.log('Private NPM Modules');
		 ...
		 */
			...
			extensionFilePath = path.join(path.resolve(__dirname, (lessThanNodeV6 || initial_install===false) ? '../../../content/config/' : '../../../node_modules/periodicjs/content/config/'), 'extensions.json');
		}
		/**
		...
		if (extensionSkipModifyConf !== true || appsettings.skipconffile !== true) {
			// console.log('extensionFilePath',extensionFilePath);
	...
	// console.log('pre init appsettings',appsettings);
...
Extensions.prototype.install = function (options, callback) {
	...
	if(dirname.match('periodicjs/node_modules/@')){
		extpublicdir = path.resolve(dirname, '../../../public/extensions/', extname);
		extconfigdir = path.resolve(dirname, '../../../content/config/extensions/', extname);
	}
...
Extensions.prototype.uninstall = function (options, callback) {
	...
	initial_install = (typeof options.initial_install !=='undefined' || typeof options.initial_install ==='boolean' ) ? options.initial_install : initial_install;
