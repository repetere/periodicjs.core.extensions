...
Extensions.prototype.moveExtensionBefore = function (options, callback) {
	...
	this.getExtensions({}, function (err, currentExtensionsConf) {
		if (err) {
			...
			else if ((indexofmovebefore === false || indexofext === false) && (typeof installatindex !=='number')) {
				callback(null, {
					message: 'MOVE BEFORE ERROR: trying to move extensions that are not in configuration'
				});
			}
			else {
				// console.log('installatindex',installatindex,'indexofmovebefore',indexofmovebefore,'indexofext',indexofext);
			...
			this.updateExtConfFile(results.installExtSetPeriodicConf,
				function (err, status) {
					if (err) {
						...
						if (movebefore || typeof installatindex==='number') {
